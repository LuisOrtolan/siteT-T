...
create table if not exists public.sala_desenhos (
  ...
  tipo text not null,                            -- 'traco' | 'linha' | 'retangulo' | 'trapezio' | 'circulo' | 'triangulo' | 'pentagono' | 'hexagono' | 'texto'
  pontos jsonb not null,                          -- array de {x,y} fracionários (0..1) — pra 'texto', 1 ponto só (o centro da caixa)
  cor text not null default '#c7a25a',
  espessura int not null default 3,               -- largura do traço, OU tamanho da fonte em px do mundo quando tipo='texto'
  texto text,                                     -- conteúdo, só usado quando tipo='texto' (largura/altura da caixa são medidas no cliente, nunca guardadas aqui)
  created_at timestamptz not null default now()
);
